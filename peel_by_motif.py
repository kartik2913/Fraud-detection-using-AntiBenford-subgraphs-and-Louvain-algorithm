...
def peel_by_motif(
    node_dict, fib_heap, motif_lists, least_nodes, log=True, log_interval=1
):
    total_degree = sum(
        [len(motif_tuple[0]) * motif_tuple[1] for motif_tuple in motif_lists]
    )
    n = node_dict.__len__()
    ...
    result_motif_nums = [
        len(motif_lists[i][0]) - loss_list[i] for i in range(len(motif_lists))
    ]
    ...
    for i in range(n - least_nodes):
        ...
        for motif_type_index in range(len(node_dict[node_to_remove][0].motif_list)):
            for motif_index in node_dict[node_to_remove][0].motif_list[
                motif_type_index
            ]:
                ...
                for each_node in motif_lists[motif_type_index][0][motif_index]:
                    if each_node != node_to_remove:
                        fib_heap.decrease_key(
                            node_dict[each_node][1],
                            node_dict[each_node][1].key
                            - motif_lists[motif_type_index][1],
                        )
                        node_dict[each_node][0].motif_list[motif_type_index].remove(
                            motif_index
                        )
        ...
        if log and i % log_interval == 0:
            ...
            print(
                fib_heap.total_nodes,
                avg_degree,
                max_avg,
                [
                    len(motif_lists[i][0]) - loss_list[i]
                    for i in range(len(motif_lists))
                ],
            )
            ...
            for edge_index in node_dict[node_to_remove][0].motif_list[motif_type_index]:
                ...
                for each_node in edge_list[edge_index][:-1]:
                    if each_node != node_to_remove:
                        fib_heap.decrease_key(
                            node_dict[each_node][1],
                            node_dict[each_node][1].key - edge_list[edge_index][-1],
                        )
                        try:
                            node_dict[each_node][0].motif_list[motif_type_index].remove(
                                edge_index
...
def several_peel_weighted(
    edges, node_chisquares, peel_times=3, log=True, weight_by="average"
):
    pairs = []
    removed_nodes = []
    for edge in edges:
...
def multipass_greedy_peel(
    G, pos_list_generator, pos_value, neg_list_generator, neg_value
):
    density_list = []
    H = G.copy()
